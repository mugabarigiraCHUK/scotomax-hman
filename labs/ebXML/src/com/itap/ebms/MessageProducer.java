...
import org.itap.http.ssl.client.*;
/**
 * 
 * @author ITAP
 * @version 1.0.0
...
public class MessageProducer {
	...
	public String produce(String conversationId, 
		...
		SimpleDateFormat runno = new SimpleDateFormat("yyyyMMdd-HmsS");
		String messageId = runno.format( new Date() );
		...
		try {
			...
			try {
				ebPayload = soapProducer.buildLicensePerInvoiceMsg(DOCUMENT_REF_NAME, 
																	messageId, 
																	license);
//				logger.debug("Payload Msg: " + ebPayload);

				logger.debug(" *** Sign SOAP> "+IP_ADDRESS+":"+ SERVICES_PORT+"; "+ 
						"ca>"+ KEYSTORE_PATH +","+ KEYSTORE_PASSWORD +";...");
				ebPayload = SignatureGenerator.digitalSign(KEYSTORE_PATH, 
														KEYSTORE_PASSWORD, 
														KEYSTORE_ALIASNAME, 
														ALIASNAME_PASSWORD, 
														ebPayload, null);
				logger.debug("Payload Msg+Signature: " + ebPayload);
//				System.out.print("<<ebPayload>>" + ebPayload);
			} catch ( Exception e ) {
				e.printStackTrace();
			...
			// -------- 2 way SSL -------
			logger.debug(" *** SSL Authenticate> "+IP_ADDRESS+":"+ SERVICES_PORT+"; "+ 
					"client>"+ TRUST_STORE_PATH +","+ TRUST_STORE_PASSWORD +";"+
					"server>"+ TRUST_STORE_PATH +","+ TRUST_STORE_PASSWORD +";...");

			// Create object for HTTP SSL communication
			SSL2WaysClient client = new SSL2WaysClient();

			// Create HTTP SSL connection pipe by 2 way authentication
			logger.debug(" *** Socket hand shaking...");
			client.createConnection(IP_ADDRESS, SERVICES_PORT, 
...
//			wr = new BufferedWriter(new OutputStreamWriter(socket.getOutputStream(), "ISO-8859-1")); 
			
			StringBuffer ebMS = new StringBuffer();
			
			ebMS.append("--boundaryValue" + CR); 
			ebMS.append("Content-ID: <soappart>" + CR);
			ebMS.append("Content-Type: text/xml; charset=\"ISO-8859-1\"" + CR);
			
			ebMS.append( CR );
			// Build MessageHeade
			logger.debug(" *** Build XML MessageHeade...");
			ebMS.append("<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?>" + CR);
			...
			ebHeader.append("			</eb:MessageData>" + CR);
			ebHeader.append("			<eb:DuplicateElimination/>" + CR);
			...
			String unrequired = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\" standalone=\"no\"?>";
			ebMS.append( signedHeader.substring(unrequired.length(), signedHeader.length()) );
			*/
			ebMS.append( ebHeader.toString() );
...
/*            String szHttpHeader="";
			...
			szHttpHeader += "POST " + SERVICES_PATH + " HTTP/1.1" + CR;
			szHttpHeader += "Pragma: no-cache" + CR;
			szHttpHeader += "Cache-Control: no-cache" + CR;
			szHttpHeader += "Cache-Control: max-age=0" + CR;
			szHttpHeader += "Host: "+IP_ADDRESS+":"+SERVICES_PORT + CR;
			szHttpHeader += "SOAPAction: \"ebXML\"" + CR;
			szHttpHeader += "Content-Type: multipart/related; type=\"text/xml\"; boundary=\"boundaryValue\" "+ CR;
			szHttpHeader += "Content-Length: " + ebMS.toString().length() + CR;
			wr.write( szHttpHeader + CR );
*/			
...
//			wr.flush();  
			...
			// -------- 2 way SSL -------
			String strURL = SERVICES_PATH;
			String strSoapAction = "ebXML";
//			String ebXML = "<soap>...</soap><payload>...</payload>"; // String of SOAP <- ebXML <- Payload
			String contentType = "multipart/related; type=\"text/xml\"; boundary=\"boundaryValue\" ";

			// Send ebXML request and receive ebXML response
			String s_ebMS= URLDecoder.decode( ebMS.toString(), "ISO-8859-1"); 
			logger.debug( "..SOAP Sending: " + s_ebMS );
			HttpServerResponse response = client.communicate(strURL, strSoapAction, s_ebMS, contentType);

			// Parse result
			logger.debug( "..SOAP Received: " + response.getResponseStatusCode() + ": " + response.getResponseBody());
			ret= response.getResponseBody();			
//			ret = URLDecoder.decode( ret, "ISO-8859-1"); 
			// -------- 2 way SSL -------

			
/*			logger.debug( szHttpHeader + CR + ebMS.toString() );
			...
			if ( in != null ){
				...
				ret = URLDecoder.decode( results.toString(), "ISO-8859-1");				
			} 
*/
	...
	public String cancel(String conversationId, Integer minutesToLeave, LicenseCancel license) throws Exception {
		...
		try {
			...
			*/
			...
			szHttpHeader += "Cache-Control: max-age=0" + CR;
			szHttpHeader += "SOAPAction: \"ebXML\"" + CR;
			szHttpHeader += "Content-Type: multipart/related; type=\"text/xml\"; boundary=\"boundaryValue\"; start=\"<ebxmlheader@ptc.doeb.go.th>\""+ CR;
			szHttpHeader += "Content-Length: " + ebMS.toString().length() + CR;
	...
	String EBXML_ENVELOP_SIGNATURE;
	...
	public void setEBXML_ENVELOP_SIGNATURE(String eBXML_ENVELOP_SIGNATURE) {
		EBXML_ENVELOP_SIGNATURE = nullToEmpty( eBXML_ENVELOP_SIGNATURE );
	}
